...
class Shop extends handy.system.BaseClass {
  ...
  uninstallApp(){
    ...
    .then(()=>{
      // post uninstall functions e.g. get store details, theme modifications, install script tags, etc
      const postAppUninstallFunctions = handy.system.systemGlobal.get('handy_shopify_postAppUninstallFunctions') || [];
      const promiseFactory = (postFunction)=>{
        return postFunction.bind(this)()
      }

      let promiseChain = Promise.resolve();
      postAppUninstallFunctions.forEach((postFunction)=>{
        promiseChain = promiseChain.then(()=> promiseFactory(postFunction));
      })

      return promiseChain
        .then(()=> {
          // end subscription
          this.getSubscriptions({active: true})
...
console.log('WARNING: handy-shopify/getProducts has not implemented variant fields for requests that specify ids or handle')
  ...
  createRecurringCharge(planName, testFlag=false){
...
//          returnUrl: `https://${this.myshopify_domain}/admin/apps/${handy.system.systemGlobal.getConfig('shopify_api_key')}`,
          returnUrl: `${handy.system.systemGlobal.get('billing_charge_activation_redirect_url')}?shop=${encodeURIComponent(this.myshopify_domain)}&billing_plan=${encodeURIComponent(plan.name)}${(testFlag || plan.test) ? '&test_flag=true' : ''}`,
          test: (testFlag || plan.test) ? true : false,
          lineItems: [
            {
              plan: {
...
/*
      return new Promise((resolve, reject)=>{
        let recurring_application_charge = {
          name: plan.name,
          price: plan.price,
          return_url: handy.system.systemGlobal.get('billing_charge_activation_redirect_url') + '?billing_plan=' + encodeURIComponent(plan.name)
        }

        if(plan.trial_length){
          recurring_application_charge.trial_days = plan.trial_length;
        }

        if(testFlag || plan.test){
          recurring_application_charge.test = true;
          recurring_application_charge.return_url += '&test_flag=true'
        }

        const payload = {recurring_application_charge};
        const shopify_api_version = handy.system.systemGlobal.getConfig('shopify_api_version');
        this.createShopifyAPI();
        this.Shopify.post('/admin/api/' + shopify_api_version + '/recurring_application_charges.json', payload, (err, data, headers)=>{
          if(err){return reject(new Error('createRecurringCharge: error creating new charge -\n', err.message)); }
          const {status, confirmation_url} = data.recurring_application_charge;
          if(status !== 'pending'){return reject(new Error('createRecurringCharge: error creating new charge - charge status: ' + status)); }
          return resolve(confirmation_url);
        })
      })
*/
      ...
      const promiseFactory = (tagId)=>{
        return new Promise((resolve, reject)=>{
          this.Shopify.delete('/admin/api/' + shopify_api_version + '/script_tags/' + tagId + '.json', (err, data, headers)=>{
            if(err){return reject(new Error('resetScriptTags: error removing script tag - id: ' + tagId + ' - err: ' + JSON.stringify(err)))}
            return resolve();
          })
        })
      }
...
/*
 * create array of functions (promises) to be executed whenever a shop uninstalls the apps
 *
 * @param {array or promise} postUninstallFunctions - array of promises (or just a single promise) to be executed
 * @api public
 */
 ...
 * @api public
 */
exports.addPostAppUninstallFunctions = addPostAppUninstallFunctions;

function addPostAppUninstallFunctions(postUninstallFunctions){
  if(!postUninstallFunctions){return;}

  let currentPostUninstallFunctions = handy.system.systemGlobal.get('handy_shopify_postAppUninstallFunctions') || [];

  if(!Array.isArray(postUninstallFunctions)){
    currentPostUninstallFunctions.push(postUninstallFunctions);
  } else {
    postUninstallFunctions.forEach((postFunction)=>{
      currentPostUninstallFunctions.push(postFunction)
    })
  }

  return handy.system.systemGlobal.set('handy_shopify_postAppUninstallFunctions', currentPostUninstallFunctions);
}
