    ...
    return new Promise((resolve, reject)=>{
      if(gql){
        ...
        this.Shopify.post('/admin/api/' + shopify_api_version + '/webhooks.json', webhook, (err, data, headers)=>{
          if(err){return reject(new Error('createWebhooks: error creating webhook ' + webhook.webhook.topic + ' - ' + JSON.stringify(err))); }
...
function authenticateShopifyRequest(req, res, next){
  handy.system.log({req, level: 'info', category: 'system', msg: 'Begin of authenticateShopifyRequest'});
...
function _authenticateRegularShopifyRequests(req, res, next){
  ...
  handy.system.log({req, level: 'info', category: 'system', msg: 'Begin of _authenticateRegularShopifyRequests'});
  let query = req.query;
  ...
  .then(()=>{
//    shop.createShopifyAPI();
    handy.system.log({level: 'info', category: 'system', msg: 'Before of isValidSignature'});
    const requestVerified = shop.isValidSignature(queryClone, true);
    handy.system.log({level: 'info', category: 'system', msg: 'After of isValidSignature'});
//    const requestVerified = shop.Shopify.is_valid_signature(queryClone, true);
  ...
  .catch((err)=> {
    ...
    handy.system.log({level: 'error', category: 'system', msg: 'error authenicating shopify regular request', shop: shop.myshopify_domain, err});
  })

  handy.system.log({level: 'info', category: 'system', msg: 'End of _authenticateRegularShopifyRequests'});
}
...
function _internalUrlHashValidation(query){
  handy.system.log({level: 'info', category: 'system', msg: 'Begin of _internalUrlHashValidation'});
...
function installShop(req, res, next){
  ...
  handy.system.log({req, level: 'info', category: 'system', msg: 'installShop begin'});
  ...
  .then((installedFlag)=> {
    ...
    handy.system.log({req, level: 'info', category: 'system', msg: 'Before begin beginShopifyInstall'});
    return installShop.beginShopifyInstall()
    .then((shopifyAuthUrl)=>{

      handy.system.log({req, level: 'info', category: 'system', msg: 'In beginShopifyInstall got shopifyAuthUrl', shopifyAuthUrl});
...
*/
    ...
    .then((shopifyAuthUrl)=> 
    {
      handy.system.log({req, level: 'info', category: 'system', msg: 'In beginShopifyInstall before redirect to  shopifyAuthUrl', shopifyAuthUrl});
